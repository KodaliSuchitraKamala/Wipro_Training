...
print("Coefficents: ", ridge.coef_)
print("MSE: ", mean_squared_error(y_test, y_pred))
x1_test = x_test[:, 0]
x2_test = x_test[:, 1]
fig = plt.figure(figsize = (10, 8))
ax = fig.add_subplot(111, projection = '3d')
ax.scatter(x1_test, x2_test, y_test, color = '#0000FF', label = 'Actual', alpha = 0.8)
ax.scatter(x1_test, x2_test, y_pred, color = '#FF0000', label = 'Predicted', alpha = 0.8)
...
ax.legend()
def rotate(angle):
    ax.view_init(elev=30 + 10*np.sin(np.radians(angle)), azim=angle) 
ani = FuncAnimation(fig, rotate, frames = np.arange(0, 360, 2), interval = 5)
plt.show()